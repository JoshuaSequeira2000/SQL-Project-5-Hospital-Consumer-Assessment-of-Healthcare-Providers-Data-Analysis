...
-- 3) Total Number of Facilities (Hospitals) registered within database for each state and the total count.
...
GO
-- Communication about medicines dropped from 65% to 61% from 2015 to 2023 respectivley.
GO
...
-- 6a) Average overall top box percentage for each state. (Top 5)
...
-- 6b) Average overall top box percentage for each state. (Bottom 5)
with State_Average_TBP as
(
	select sr.State, s.State_Name, AVG(sr.Top_box_Percentage) as Avg_Top_Box_Percentage, 
...
-- 6d) Total number of hospitals registered per state.
with number_of_hospitals as
(
	select State, COUNT(distinct Facility_ID) as Number_Of_Hospitals
	from Responses
	group by State
)
select * from number_of_hospitals
order by Number_Of_Hospitals desc
GO

-- 6e) Avg top box percentage for all states with all measures from 2015 to 2023
